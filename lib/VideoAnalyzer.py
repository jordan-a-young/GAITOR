...
"""
class VideoAnalyzer():
    def __init__(self, filepath, StackedWidget, should_rotate=False):
        ...
        self.StackedWidget.detectionLabel.setStyleSheet('color: RED')
    ...
    def analyze(self):
        ...
        self.StackedWidget.quitButton.clicked.connect(lambda: self.quit(unified, last_frame))
        ...
        while True:
            if frame_numb % 50 is 0:
                self.StackedWidget.detectionLabel.setText('Undetected')
                self.StackedWidget.detectionLabel.setStyleSheet('color: RED')
                self.StackedWidget.circumferenceLabel.setText('0')
                self.StackedWidget.calcAreaLabel.setText('0')
                self.StackedWidget.contourAreaLabel.setText('0')
                self.StackedWidget.hullAreaLabel.setText('0')
                self.StackedWidget.xCoordLabel.setText('0')
                self.StackedWidget.yCoordLabel.setText('0')
            ...
            try:
                ...
                if len(contours) > 0:
                    if self.StackedWidget.detectionLabel.text() == 'Undetected':
                        self.StackedWidget.detectionLabel.setText('Detected')
                        self.StackedWidget.detectionLabel.setStyleSheet('color: GREEN')
                        self.StackedWidget.detectionLabel.update()

                    for i, cnt1 in enumerate(contours):
                        if self.contourCount % 10 is 0:
                            self.StackedWidget.circumferenceLabel.setText('%d' % cv2.arcLength(cnt1, True))
                            self.StackedWidget.calcAreaLabel.setText('%d' % (cv2.arcLength(cnt1, True)/(2*math.pi))**2)
                            self.StackedWidget.contourAreaLabel.setText('%d' % cv2.contourArea(cnt1))
                    ...
                    for i in cloud_numbers:
                        ...
                        if pos.size != 0:
                            ...
                            if (cv2.contourArea(hull) > 100 and cv2.contourArea(hull) < 1000):
                                if self.contourCount % 10 is 0:
                                    self.StackedWidget.hullAreaLabel.setText('%d' % cv2.contourArea(hull))
                                    self.StackedWidget.xCoordLabel.setText('%d' % int(M['m10'] / M['m00']))
                                    self.StackedWidget.yCoordLabel.setText('%d' % int(M['m01'] / M['m00']))
                    ...
                    self.contourCount = self.contourCount + 1
                else:
                    self.contourCount = 0
    ...
    def quit(self, unified, last_frame):
        # Cleanup
        self.video.release()
        ...
        count = 10
        while count >= 0:
            self.StackedWidget.countdownLabel.setText(str(count))
            self.StackedWidget.countdownLabel_2.setText(str(count))
            self.StackedWidget.repaint()
            count = count - 1
            time.sleep(1)
